...
namespace Trellow.ViewModels.Boards
{
    ...
    public class BoardViewModel : PivotViewModel
    {
        ...
        private static IScreen FindSelectedItem(IList<IScreen> items, string selectedListId)
        {
            ...
            {
                var found = items.OfType<BoardListViewModel>()
                                 .FirstOrDefault(item => item.Id == selectedListId);
                if (found != null)
                    selectedIndex = items.IndexOf(found);
            }
        ...
        private void InitializeLists(List<List> lists)
        {
            var vms = lists.Select(list => _listFactory()
                                               ...
                                               .Bind(AppBar)).Cast<IScreen>(); // necessary for older c#
